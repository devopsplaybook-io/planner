...
- [x] The shared project selection component renders the projects as a searchable, collapsible tree: sub-projects are indented under their parent, each entry shows its last name segment with the full path (e.g. Home / Parents) as secondary text and tooltip, and the search matches any segment of the path; orphaned projects are shown at the root with a warning marker.
